...
#if defined(NDEBUG)
# error "Hancoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xe37d1e9702cc76d9b08a75c2e252540f17e8a4fc6cf7c64afa00dce73fa8af78");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Hancoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Hancoin Signed Message:\n";
...
{
    // Hancoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    int64 nSubsidy = 236 * COIN;

            if(nHeight == 1)
            {
            nSubsidy = 103651200 * COIN;
            }

    // Subsidy is cut in half every 610000 blocks
    nSubsidy >>= (nHeight / 610000);
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Hancoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Hancoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1533938476, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        // Genesis block
        const char* pszTimestamp = "The rebirth of the coin of revenge";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 236 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("041922e441a16096726747e7642f06867be754840ccce975c7f922db6cdaa811b95e8b8400f46b72f9d5fbfa98a0a67346872a814ac3024bf19417de460f47a355") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1533938476;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1350993;
        ...
        assert(block.hashMerkleRoot == uint256("0xd309ffcc96a3940749290a21bf90bc109855d0d0d12e5af8e3b9587158c201c9"));
...
unsigned char pchMessageStart[4] = { 0x68, 0xcf, 0x7b, 0x8a }; // Hancoin: increase each by adding 2 to bitcoin's value.
...
CBlockTemplate* CreateNewBlock(const CScript& scriptPubKeyIn)
{
    ...
    int64 nFees = 0;
    {
        ...
        while (!vecPriority.empty())
        {
            ...
            // transactions:
            if (!fSortedByFee &&
                ((nBlockSize + nTxSize >= nBlockPrioritySize) || (dPriority < COIN * 480 / 250)))
            {
                fSortedByFee = true;
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("HancoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("HancoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static HancoinMiner(CWallet *pwallet)
{
    printf("HancoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("hancoin-miner");
    ...
    try { loop {
        ...
        printf("Running HancoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("HancoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&HancoinMiner, pwallet));
}
